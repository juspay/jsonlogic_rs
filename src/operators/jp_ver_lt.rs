...
pub fn compute(args: &[Expression], data: &Data) -> Value {
    ...
    let result = compare_version(&first, &second, true).is_lt()
        && third.map_or(true, |third| compare_version(&second, &third, true).is_lt());
...
pub fn partial_compute(args: &[Expression], data: &Data) -> PartialResult {
    ...
    let result = match args.get(2) {
        None => compare_version(&first?, &second?, true).is_lt(),
        Some(third_arg) => {
            let third = third_arg
            ...
            match (first, second, third) {
                (Ok(first), Ok(second), Ok(third)) => {
                    compare_version(&first, &second, true).is_lt()
                        && compare_version(&second, &third, true).is_lt()
                }
                (Ok(arg1), Err(Ambiguous), Ok(arg2))
                | (Err(Ambiguous), Ok(arg1), Ok(arg2))
                | (Ok(arg1), Ok(arg2), Err(Ambiguous))
                    if !compare_version(&arg1, &arg2, true).is_lt() =>
                {
